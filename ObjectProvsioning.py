...
def getAuthToken(ECSNode, User, Password):
    curlCommand = "/bin/curl -i -k https://%s:4443/login -u %s:%s" % (ECSNode, User, Password)
...
def executeRestAPI(url, method, filter, data, ECSNode,contentType='json',checkOutput=0):
    ...
    if "license" in url:
        data="-T /vagrant/license.xml"
    curlCommand = "/bin/curl -s -k -X %s -H 'Content-Type:application/%s' \
